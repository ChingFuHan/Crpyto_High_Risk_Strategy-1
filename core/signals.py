...
def analyze(symbol: str, timeframe: str = '5m', limit: int = 100) -> Signal:
    ...
    if price > latest.get('high_20', price) * 0.998:  # near/above 20-bar high
        if vol_ratio >= cfg_long.volume_multiplier:
            if rsi > cfg_long.rsi_breakout:
                direction = 'LONG'
                setup = 'A'
                strength = min(1.0, 0.5 + (vol_ratio / 10) + (rsi - 60) / 100)
                reasons.append(f"Breakout above 20-bar high, vol_ratio={vol_ratio:.1f}, RSI={rsi:.1f}")
    ...
    if direction == 'NEUTRAL':
        if prev['high'] >= prev.get('high_20', prev['high']) * 0.998:
            if price < prev['high'] and price < latest['open']:
                if vol_ratio >= 2.0:
                    direction = 'SHORT'
                    setup = 'C'
                    strength = min(1.0, 0.5 + vol_ratio / 10)
                    reasons.append(f"Failed breakout, rejected from high, vol_ratio={vol_ratio:.1f}")
